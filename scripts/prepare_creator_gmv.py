...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    table_payload = {"sheets": [
        make_sheet(cleaned_output, names["monthly_cleaned"]),
        make_sheet(summary, names["monthly_summary"], "A7"),
        make_sheet(cleaned_output, names["latest_cleaned"]),
        make_sheet(summary, names["latest_summary"], "A7"),
    ]}
    styles_payload = {"styles": [
        style_for_sheet(names["monthly_cleaned"], len(cleaned_output), len(cleaned_output.columns), False),
        style_for_sheet(names["monthly_summary"], len(summary), len(summary.columns), True),
        style_for_sheet(names["latest_cleaned"], len(cleaned_output), len(cleaned_output.columns), False),
        style_for_sheet(names["latest_summary"], len(summary), len(summary.columns), True),
    ]}
