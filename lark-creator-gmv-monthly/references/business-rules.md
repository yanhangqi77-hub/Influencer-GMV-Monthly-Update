...
执行 `commission-incentives.md` 和 `commission-policy.json`。激励按当月已付款扣除取消及退款/退货的净件数，按单个产品（503/303/509）分别累计并各自选档，不是所有产品合计件数，也不是Total Orders、GMV或未扣售后的SKU数量。含303和503的组合SKU同时计入两者。退款金额不按比例换算多件订单退款件数；不明情况待核实。

人工列保护的唯一新增例外：X备注中的 `[GMV佣金激励] ... [/GMV佣金激励]` 为脚本自有区块，每轮替换或清除；其余人工文字保留。R:W真实佣金与合作信息完全不由激励脚本改动。候选备注“涨佣金为xx”是目标比例并注明资格/预算待审核，不代表已审批或已生效。
