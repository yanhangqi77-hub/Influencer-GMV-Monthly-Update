...
from commission_incentives import apply_incentives, conditional_formats
...
def iter_table_nodes(value: Any) -> Iterable[dict[str, Any]]:
    if isinstance(value, dict):
        if "annotated_csv" in value:
            if value.get("has_more") or value.get("truncated"):
                raise ValueError("Manual source was truncated; read the complete summary first")
            raw = re.sub(r"(?m)^\[row=\d+\] ?", "", value["annotated_csv"])
            records = list(csv.reader(io.StringIO(raw)))
            header = next((i for i, row in enumerate(records) if any(norm(c) in {"creatorhandle", "creatorusername"} for c in row)), None)
            if header is not None:
                yield {"columns": records[header], "data": records[header + 1:]}
            return
...
def load_manual_sources(paths: list[Path], issues: list[str]) -> dict[str, dict[str, Any]]:
    ...
    for path in paths:
        ...
        tables = list(iter_table_nodes(payload))
        if not tables:
            issues.append(f"Unsupported manual source format: {path.name}; use columns/data or complete annotated_csv")
        for table in tables:
        ...
        for creator_key, values in source_map.items():
            merged.setdefault(creator_key, {}).update(values)
    return merged
...
def json_ready_frame(frame: pd.DataFrame) -> list[list[Any]]:
    ...
    for column in clean.columns:
        ...
        elif pd.api.types.is_string_dtype(clean[column]):
            # Empty text cells must serialize as "" so an overwrite clears stale
            # values (table-put skips null cells instead of clearing them).
            clean[column] = clean[column].where(clean[column].notna(), "")
...
def make_sheet(frame: pd.DataFrame, name: str, start_cell: str = "A1") -> dict[str, Any]:
    ...
    manual_integer_columns = {"503", "303", "509"}
    for column in frame.columns:
        ...
        if column in identifier_columns or norm(column).endswith("id"):
            ...
            dtypes[column] = "Int64"
            formats[column] = "#,##0"
        elif column in manual_integer_columns:
            # Manual rate columns are written as text so empty cells serialize as
            # "" (clearing stale values) rather than null (which overwrite skips).
            dtypes[column] = "object"
            formats[column] = "@"
...
def style_for_sheet(name: str, rows: int, columns: int, summary: bool) -> dict[str, Any]:
    ...
    style: dict[str, Any] = {
        ...
        "freeze": {"rows": start_row, "cols": 2 if summary else 0},
    }
    if summary and rows:
        manual_styles = [{
            "range": "R1:X1",
            "background_color": "#EBD8EF",
            "font_color": "#262626",
        }]
        if rows > 1:
            manual_styles.append({
                "range": f"A2:X{last_row - 1}",
        ...
        style["cell_styles"].extend(manual_styles)
    return style
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    cleaned_paid = cleaned[resolved["paid_time"]].dropna()
    if cleaned_paid.empty:
        raise ValueError(f"No eligible orders remain for {selected_period} after status cleaning")
    range_start = cleaned_paid.min()
    range_end = cleaned_paid.max()
    latest_range_label = f"{range_start:%y%m%d}~{range_end:%y%m%d}"
    ...
    cleaned["__incentive_quantity_raw__"] = cleaned[resolved["quantity"]].copy()
    ...
    incentive_audit = apply_incentives(
        cleaned, resolved, metric_rows, str(selected_period),
        getattr(args, "creator_context", None),
    )
    if incentive_audit["counts"].get("needs_review"):
        warnings.append("Some commission recommendations require refund, identity or current-rate verification; see commission-incentive-audit.json")
    ...
    output_columns = [
        ...
        if column not in {"__source_row__", "__creator_key__", "__incentive_quantity_raw__"}
    ...
    monthly_label = args.month_label or str(selected_period)
    names = {
        "monthly_cleaned": f"Cleaned Orders - {monthly_label}",
        "monthly_summary": f"Creator GMV Summary - {monthly_label}",
        "latest_cleaned": f"Cleaned Orders - {latest_range_label}",
        "latest_summary": f"Creator GMV Summary - {latest_range_label}",
    }
    ...
    tables_by_key = {
        ...
        "monthly_summary": make_sheet(summary, names["monthly_summary"]),
        "latest_cleaned": make_sheet(cleaned_output, names["latest_cleaned"]),
        "latest_summary": make_sheet(summary, names["latest_summary"]),
    }
    styles_by_key = {
    ...
    conditional_payload = conditional_formats(
        [names[key] for key in target_keys if key.endswith("summary")], len(summary),
    )
    ...
    all_expected_ranges = {
        ...
        names["monthly_summary"]: f"A1:{column_letters(len(summary.columns))}{len(summary) + 1}",
        names["latest_cleaned"]: f"A1:{column_letters(len(cleaned_output.columns))}{len(cleaned_output) + 1}",
        names["latest_summary"]: f"A1:{column_letters(len(summary.columns))}{len(summary) + 1}",
    }
    manifest = {
        ...
        "selected_month": str(selected_period),
        "month_label": monthly_label,
        "latest_range_label": latest_range_label,
        "paid_time_range": {
        ...
        "commission_incentives": {k: v for k, v in incentive_audit.items() if k != "creators"},
    ...
    return {"manifest": manifest, "tables": table_payload, "styles": styles_payload,
            "incentives": incentive_audit, "conditional_formats": conditional_payload}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--month-label", help="Optional monthly sheet-name label in place of YYYY-MM; latest names always use the cleaned Paid Time range")
    ...
    parser.add_argument("--creator-context", type=Path, help="Optional verified creator role/eligibility/current-rate JSON; never infer premium from a 15%% rate alone")
...
def main() -> int:
    ...
    try:
        ...
        (args.output_dir / "commission-incentive-audit.json").write_text(
            json.dumps(result["incentives"], ensure_ascii=False, indent=2), encoding="utf-8"
        )
        (args.output_dir / "lark-conditional-formats-payload.json").write_text(
            json.dumps(result["conditional_formats"], ensure_ascii=False, indent=2), encoding="utf-8"
        )
        (args.output_dir / "manifest.json").write_text(
