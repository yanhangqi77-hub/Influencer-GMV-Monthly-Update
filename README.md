...
本版以当前已安装 Skill 为准，核心生成脚本保持一致；补充使用说明和虚构数据测试，并澄清人工字段读取格式。打包不会改写飞书，不包含原始订单、达人名单、佣金快照或本次生成的数据文件。

## GitHub 发布版

已移除私人飞书链接和工作表 ID。首次使用时提供自己的目标表链接，由 Codex 定位已有滚动工作表并记录实际 ID；若目标表没有滚动工作表，先明确安排初始化，再执行更新。本版无自动配置文件读取机制，不应把任意 JSON 配置文件视为已经生效。

上传时将本文件夹中的内容作为仓库内容即可。包内 .gitignore 只是辅助保护；提交前仍须检查没有加入本地订单、预览结果、授权信息或私人配置。
