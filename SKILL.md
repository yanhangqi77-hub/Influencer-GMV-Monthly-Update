...
description: "清洗 TikTok Shop 原始订单表，按付款月份生成并更新飞书中的 Cleaned Orders 与 Creator GMV Summary 月度表和最新表。当用户提供订单 Excel/CSV 并要求统计月度达人 GMV、刷新 Creator GMV Summary 或更新最新快照时使用。"
metadata:
  requires:
    bins: ["lark-cli"]
---
...
- 确认输入文件存在且可读。
- 确认目标飞书表可访问，并读取工作簿与四个目标工作表的当前状态。
- 读取已有月表或最新汇总表中的人工维护列，保存为后续合并源。
...
1. 先写月度两个工作表。
2. 核验月度工作表回读结果。
3. 确认本次月份不早于现有“最新”数据；再用完全相同的数据写入两个“最新”工作表。历史回补不得让“最新”倒退，除非用户明确要求。
4. 核验“最新”与月度快照一致。
...
- 四个目标工作表均存在且更新完成；
- 月表和“最新”表的明细行数、汇总 GMV、渠道 GMV 完全一致；
