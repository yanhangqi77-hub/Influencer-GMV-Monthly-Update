...
- 用户说“更新达人最新 GMV”时，只更新一组持续滚动的工作表。工作表名称使用本次清洗后有效订单的 `Paid Time` 最早日和最晚日，格式为：
...
目标是用户指定的飞书电子表格，通常名为 `Creator GMV Summary`。本发布版不内置私人表格链接或工作表 ID。首次使用须取得用户指定的目标链接；只有标题时按标题精确定位，歧义时请用户选择。

在目标工作簿中定位原 `- 最新` 工作表，或唯一一对同日期标签、名称符合 `Cleaned Orders - \d{6}~\d{6}` 与 `Creator GMV Summary - \d{6}~\d{6}` 的工作表，并记录真实 `sheet_id` 供后续滚动更新。不同工作簿不得复用 ID。无法唯一定位时停止，不得新建或猜测；若尚未初始化，由用户明确指定初始化任务后再处理。
